...
**逆位一律把牌图旋转 180°**，全站 5 处渲染点（`CardImage` / `ReadingSummary` /
`/notes` / `/notes/[id]` / `VideoSceneRenderer`）保持一致。

历史上 `DrawingStage` 单独不转，注释理由是「倒立的牌面会放大不适感」。
那条决定从未推广到其余各处，实际效果是同一张逆位牌在不同页面朝向不同。
现已统一为「都转」。若要改回「都不转」，必须五处一起改，不要只改一处。
